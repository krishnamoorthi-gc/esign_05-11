...
if (smtpenable) {
  ...
  // For local development, use a simple email adapter that logs to console
  isMailAdapter = true;
  console.log('Email adapter configured for local development - emails will be logged to console');
}

const mailsender = smtpenable
  ? process.env.SMTP_USER_EMAIL
  : process.env.MAILGUN_SENDER || 'noreply@opensign.com';
export const config = {
  databaseURI:
  ...
  publicServerURL: process.env.PUBLIC_SERVER_URL || process.env.SERVER_URL || cloudServerUrl,
          ...
          options: {
            ...
            apiCallback: async ({ payload, locale }) => {
              if (mailgunClient) {
                ...
                // For local development, just log the email to console
                console.log('Email would be sent:', JSON.stringify(payload, null, 2));
              }
            },
          },
        },
