...
export default async function UpdateExistUserAsAdmin(request) {
  ...
  const domain = request.params.domain; // Add domain parameter
  try {
    if (masterkey !== process.env.MASTER_KEY) {
      throw new Parse.Error(404, 'Invalid master key.');
    }
    
    // Check for admin within the same domain/tenant only
    ...
    // If domain is provided, only check within that tenant
    if (domain) {
    ...
    if (extAdminRes && extAdminRes.length > 0 && extAdminRes?.[0]?.get('OrganizationId')) {
      throw new Parse.Error(
        Parse.Error.DUPLICATE_VALUE,
        'Admin already exists for this organization. Please login to the application using admin credentials in order to manage users.'
      );
    } else {
      ...
      if (extRes) {
        ...
        if (!extUser.get('TrialStartDate')) {
          extUser.set('TrialStartDate', new Date());
        }
