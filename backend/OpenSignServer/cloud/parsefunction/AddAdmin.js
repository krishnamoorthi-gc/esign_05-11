...
async function saveUser(userDetails) {
  ...
  if (userRes) {
    ...
    // Check if email is verified (unless it's an admin user)
    ...
    // Check if serverUrl is properly configured
    if (!serverUrl || serverUrl === 'http://localhost:8081/app') {
      // If we're in a development environment or serverUrl is not configured properly,
      // we should handle this gracefully
      console.log('Server URL not properly configured for loginAs endpoint');
      // Return the existing user info instead of trying to call loginAs
    ...
    try {
      ...
      console.log('Error in loginAs call:', error.message);
      ...
      if (
        ...
        console.log('Network error detected in loginAs call, using fallback');
      }
      // Fallback to existing user session token
      return { id: userRes.id, sessionToken: userRes.getSessionToken() };
    }
  } else {
    ...
    // Automatically verify email for admin users since they are created by the system
    ...
    try {
      ...
      if (error.code === Parse.Error.ACCOUNT_ALREADY_EXISTS) {
        throw new Parse.Error(
          Parse.Error.ACCOUNT_ALREADY_EXISTS,
...
export default async function AddAdmin(request) {
  ...
  if (!userDetails || !userDetails.email || !userDetails.name || !userDetails.company) {
    throw new Parse.Error(
      Parse.Error.INVALID_QUERY,
      'Missing required parameters: email, name, and company are required.'
    );
  }
  ...
  try {
    ...
    // If we have tenant information, check within that tenant only
    if (userDetails.company) {
      const tenantQuery = new Parse.Query('partners_Tenant');
      tenantQuery.equalTo('TenantName', userDetails.company);
      tenantQuery.equalTo('EmailAddress', userDetails.email?.toLowerCase()?.replace(/\s/g, ''));
      const tenant = await tenantQuery.first({ useMasterKey: true });

      if (tenant) {
        extQuery.equalTo('TenantId', tenant);
      }
    }
    ...
    if (extUser) {
      ...
      // Add the domain field to properly associate tenant with domain
      partnerQuery.set(
        'Domain',
      ...
      // Set the trial start date for new admin users
    ...
    if (err.code) {
      throw err; // Re-throw Parse errors as-is
    } else {
      // Handle network and other unexpected errors
      const errorMessage = err.message || 'An error occurred while creating the admin user.';
