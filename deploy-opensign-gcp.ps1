# Deployment script for OpenSign to Google Cloud Platform (External IP approach) - PowerShell version
...
Write-Host "  OpenSign Deployment to GCP (External IPs)" -ForegroundColor Green
...
set -e
apt update && apt upgrade -y
apt install -y curl wget gnupg software-properties-common git
# Install Node.js (v18)
...
cd /opt/opensign
# Clone the specific repository (using the local files instead of cloning)
# For this deployment, we'll copy the local files to the instance
...
$instances = gcloud compute instances list --filter="name~'opensign-'" --format="value(name)"
if ($instances) {
    Write-Host "Deleting existing instances..." -ForegroundColor Yellow
