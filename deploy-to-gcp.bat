...
REM Deployment script for OpenSign to Google Cloud Platform

echo Starting OpenSign deployment to Google Cloud Platform...
...
REM For Esign Alpha project, use: esign-alpha-474811
set /p PROJECT_ID="Enter your Google Cloud Project ID [esign-alpha-474811]: "
if "%PROJECT_ID%"=="" set PROJECT_ID=esign-alpha-474811

REM Set your desired region (change if needed)
set REGION=us-central1
set ZONE=us-central1-a

REM Generate secure master key
for /f "delims=" %%a in ('openssl rand -base64 32') do set MASTER_KEY=%%a
...
REM Create Kubernetes deployment file
(
...
echo Your OpenSign application is now deployed to Google Cloud Platform!
for /f "tokens=*" %%a in ('kubectl get service opensign-frontend-service -o jsonpath^="{.status.loadBalancer.ingress[0].ip}"') do set EXTERNAL_IP=%%a
