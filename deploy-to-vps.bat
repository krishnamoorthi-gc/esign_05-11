@echo off
REM OpenSign Automated VPS Deployment Script (Windows Version)
REM Target: gcsign.medjooldatesindia.com (31.97.49.4)
REM This script automates the complete deployment of OpenSign to your Windows VPS server

TITLE OpenSign VPS Deployment

ECHO ========================================
ECHO   OpenSign Automated VPS Deployment     
ECHO   Target: gcsign.medjooldatesindia.com (31.97.49.4)
ECHO ========================================
...
ECHO Cloning OpenSign repository...
REM Note: You may need to install Git for Windows first
git clone https://github.com/krishnamoorthi-gc/esign_05-11.git .
REM If Git LFS is needed for large files
git lfs install
git lfs pull
...
netsh advfirewall firewall add rule name="OpenSign HTTP" dir=in action=allow protocol=TCP localport=80
netsh advfirewall firewall add rule name="OpenSign HTTPS" dir=in action=allow protocol=TCP localport=443
...
ECHO.
ECHO Next steps:
ECHO 1. For SSL, consider using Windows Certificate Store or Let's Encrypt Windows client
ECHO 2. Update the .env files in C:\opensign\backend\OpenSignServer and C:\opensign\frontend\OpenSign with your specific configuration
ECHO 3. Consider using IIS as a reverse proxy for production deployments
ECHO.
ECHO Application will be accessible at:
ECHO   Frontend: http://gcsign.medjooldatesindia.com
ECHO   Backend API: http://gcsign.medjooldatesindia.com/app
ECHO.
ECHO PM2 commands:
